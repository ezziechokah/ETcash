    ...
    <header className="h-14 bg-surface-card border-b border-surface-border flex items-center px-6 gap-4 flex-shrink-0">
      {currentEntity && (
        <div className="text-sm text-fg-muted">
          <span className="text-fg-faint">Entity: </span>
          <span className="text-fg-secondary font-medium">{currentEntity.name}</span>
        </div>
      )}
      <div className="flex-1"/>
      <div className="text-xs text-fg-faint hidden md:block">FY {new Date().getFullYear()}</div>
      <ThemeToggle compact />
      <button className="text-fg-muted hover:text-fg-secondary transition-colors">
        <BellIcon className="w-5 h-5"/>
      </button>
      <Menu as="div" className="relative">
        <Menu.Button className="flex items-center gap-2 text-fg-muted hover:text-fg-secondary transition-colors">
        ...
        <Transition as={Fragment}
          ...
          <Menu.Items className="absolute right-0 mt-2 w-52 bg-surface-card border border-surface-border rounded-xl shadow-xl z-50 py-1 focus:outline-none">
            <div className="px-4 py-3 border-b border-surface-border">
              <p className="text-sm font-medium text-fg-secondary">{user?.username}</p>
              <p className="text-xs text-fg-muted">{user?.email}</p>
            </div>
            <Menu.Item>{({ active }) => (
              <button onClick={handleLogout}
                className={`flex items-center gap-2 w-full px-4 py-2 text-sm transition-colors ${active ? 'bg-surface-border text-red-500 dark:text-red-400' : 'text-fg-muted'}`}>
